...
The `ask <question>` command uses a local LLM for grounded answers instead of raw keyword matching. No API key, no cost, fully offline.
...
```
3. Make sure Ollama is running, then use `ask` as usual. If Ollama is unreachable, the agent falls back to extractive keyword search automatically.
## Toward Jarvis
